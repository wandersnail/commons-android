...
public class DbUtils {
	...
	public static class Column {
        public String name;
		/**
		 * {@link #INTEGER}, {@link #TEXT}, {@link #REAL}, {@link #DATE}
		 */
		...
		/**
		 * @param name 列名
		 * @param dataType {@link #INTEGER}, {@link #TEXT}, {@link #REAL}, {@link #DATE}
		 * @param defualtValue 默认值
		 */
		public Column(String name, @DataType String dataType, Object defualtValue) {
			this.name = name;
    ...
    protected static class Builder<T> {
        ...
        public T where(String column, @Operator String op, Object value) {
        ...
        public T and(String column, @Operator String op, Object value) {
            and += " and " + column + op;
            values.add(value);
            return (T) this;
        }

        public T or(String column, @Operator String op, Object value) {
            or += " or " + column + op;
            values.add(value);
            return (T) this;
        }
    ...
    public static class QureyBuilder extends Builder<QureyBuilder> {
        ...
        public Cursor build() {
            String sql = "select " + whats + " from " + table + where + and + or + groupBy + orderBy + limit + offset;
    ...
    public static void insertRecord(SQLiteDatabase db, String table, ContentValues values) {
        ...
        if (keys.size() > 0) {
            ...
            String sql = "replace into " + table + "(" + strColumn + ") values (" + strArg + ")";
            db.execSQL(sql, vals);
        }
    }
    ...
    public static<T> T getColumnValue(Class<T> clazz, Cursor cursor, String columnName, T defaultValue) {
        try {
            T obj = getColumnValue(clazz, cursor, columnName);
            if (obj == null)
                return defaultValue;
            return obj;
        } catch (Exception e) {
    ...
    public static <T> T getColumnValue(Class<T> clazz, Cursor cursor, int columnIndex, T defaultValue) {
        try {
            T obj = getColumnValue(clazz, cursor, columnIndex);
            if (obj == null)
                return defaultValue;
            return obj;
        } catch (Exception e) {
